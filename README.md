...
6. я в себе налаштував так:
   - `set COMMANDLINE_ARGS= --xformers   --medvram --no-half  --theme dark --update-check --autolaunch --ckpt-dir C:\\_AI_SD_P/models/Stable-diffusion`
       - `--xformers` - для оптимізаці відео пам'яті  - при першому запуску буде скачувати бібліотеки до 5гіг
       -  `--medvram` - (у мене пришщвидшує роботу в 5 разів), для зменшення навантаження на відеопам'ять щоб уникнути помилок нестачі пам'яті  **УВАГА** викликає помилку  при **тренуванні** нової нейромережі  **Expected all tensors to be on the same device**  при тренуванні, томупри навчанні параметр слід вимкнути
